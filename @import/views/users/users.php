<?php
	...
	if($page < $first_page){
		Templater::redirect('/users?'.http_build_query([
			'p' => $first_page,
		]));
...
<?php 	foreach($users as $user): ?>
								<div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-12s mb-4">
									<a class="text-dark" href="<?= get_user_profile_page_url($user['user_name']) ?>">
										<div class="card">
											<img src="<?= get_user_profile_image_url($user['user_email'], 512) ?>" class="card-img" alt="<?= htmlentities($user['user_name']) ?>" style="max-width: 13em2">
											<div class="card-body">
...
<?php if($first_page < $page): ?>
									<li class="page-item">
										<a class="page-link" href="/users?<?= http_build_query([ 'p' => $first_page ]) ?>" aria-label="First page">&laquo;</a>
									</li>
									<li class="page-item">
										<a class="page-link" href="/users?<?= http_build_query([ 'p' => $page - 1 ]) ?>" aria-label="Previous page">&lsaquo;</a>
									</li>
<?php endif; ?>
<?php
	// middle page
...
?>
									<li class="page-item<?= $now_page === $page ? ' active' : '' ?>">
										<a class="page-link" href="/users?<?= http_build_query([ 'p' => $now_page ]) ?>" aria-label="Page <?= htmlentities($now_page) ?>"><?= htmlentities($now_page) ?></a>
									</li>
<?php
		}
		unset($now_page);
...
<?php if($page < $last_page): ?>
									<li class="page-item">
										<a class="page-link" href="/users?<?= http_build_query([ 'p' => $page + 1 ]) ?>" aria-label="Next page">&rsaquo;</a>
									</li>
									<li class="page-item">
										<a class="page-link" href="/users?<?= http_build_query([ 'p' => $last_page ]) ?>" aria-label="Last page">&raquo;</a>
									</li>
<?php endif; ?>
								</ul>
							</nav>
